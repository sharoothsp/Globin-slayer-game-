...
hitsound = pygame.mixer.Sound("col.wav")
...
class enemy(object):
	...
	def draw(self,win):
		...
		if self.visible:
			...
			self.hitbox = (self.x + 17, self.y + 1, 29, 58)
			#pygame.draw.rect(win,(255,0,0),self.hitbox,2)
	def move(self):
		if self.vel > 0:
...
goblin = enemy(100,416,64,64,300)
goblin2 = enemy(120,416,64,64,300)
timer=0
run=True
timer2=0
while run:
	...
	if keys[pygame.K_SPACE]:
		if man.left:
			...
			facing = 1
		if len(bullets) < 5 and bulletloop==0:
			bulletsound.play()
